...
# エンティティがその近くにいた場合、近くのエンティティ一体に向けて発射/いない場合はクラウド君自身に向けて発射する！
    execute at @e[tag=D_P_Set] run tp @e[tag=D_P_Set] @e[distance=..9,sort=nearest,limit=1,tag=!D_P_Set,tag=!D_NoHit,type=!item] 
...
# 回数判定
    function formchanger:use/ex/doppel_brast/d_brack_shoot_counter
