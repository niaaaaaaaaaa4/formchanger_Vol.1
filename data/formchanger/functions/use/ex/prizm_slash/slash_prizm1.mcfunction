...
# 実行座標を召喚！＆パーティクル・ヒット
    execute as @e[tag=Slash_P,tag=!Slash_5] at @s positioned ^ ^ ^3 positioned ~ ~1 ~ rotated as @e[tag=Slash_P,tag=!Slash_5] positioned ^ ^ ^0.1 rotated as @e[tag=Slash_P,tag=!Slash_5] positioned ^ ^ ^0.2 rotated as @e[tag=Slash_P,tag=!Slash_5] positioned ^ ^ ^0.4 rotated as @e[tag=Slash_P,tag=!Slash_5] positioned ^ ^ ^0.8 rotated as @e[tag=Slash_P,tag=!Slash_5] positioned ^ ^ ^1.6 unless entity @e[tag=Slash_5,distance=..3.9] if entity @s[distance=3.2..3.4] run particle end_rod ~ ~ ~ 0 0 0 0.0001 2 normal
    execute as @e[tag=Slash_P,tag=!Slash_5] at @s positioned ^ ^ ^3 positioned ~ ~1 ~ rotated as @e[tag=Slash_P,tag=!Slash_5] positioned ^ ^ ^0.1 rotated as @e[tag=Slash_P,tag=!Slash_5] positioned ^ ^ ^0.2 rotated as @e[tag=Slash_P,tag=!Slash_5] positioned ^ ^ ^0.4 rotated as @e[tag=Slash_P,tag=!Slash_5] positioned ^ ^ ^0.8 rotated as @e[tag=Slash_P,tag=!Slash_5] positioned ^ ^ ^1.6 unless entity @e[tag=Slash_5,distance=..3.9] if entity @s[distance=2.2..3.15] run particle cloud ~ ~ ~ 0 0 0 0.01 1 normal

    playsound block.amethyst_block.hit player @a ~ ~ ~ 1.9 0.75 
    
    execute at @s as @e[distance=1.8..3.6,type=!player] run tag @s add A
    execute at @s as @e[distance=3.3..3.8,type=!player] run tag @s add C
    execute at @e[tag=Slash_5] as @e[distance=3.9..9,type=#formchanger:enemy] run tag @s add B
